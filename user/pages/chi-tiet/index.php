    ...
    <div class="row">
        <div class="col-md-6">
            <div class="carousel-wrapper">
                ...
                <?php for ($i = 1; $i < count($anhSp__Get_By_Id_Sp_Not_First); $i++): ?>
                    <label for="slide<?= $i ?>" class="nav-dot">
                        <img src="../assets/<?= $anhSp__Get_By_Id_Sp_Not_First[$i]->hinhanh ?>" class="item" alt="">
                    </label>
                <?php endfor ?>

                <?php for ($i = 1; $i < count($anhSp__Get_By_Id_Sp_Not_First); $i++): ?>
                    <label
                        for="slide<?= ($i - 1 + count($anhSp__Get_By_Id_Sp_Not_First)) % count($anhSp__Get_By_Id_Sp_Not_First) ?>"
                        class="left-arrow">❮</label>
                    <label for="slide<?= ($i + 1) % count($anhSp__Get_By_Id_Sp_Not_First) ?>" class="right-arrow">❯</label>
                <?php endfor ?>
        ...
        <div class="col-md-5 info-right">
            ...
            <div>
                <label>
                    <h2 class="product-name"><?= $sp__Get_By_Id->tensp ?></h2>
                </label>
            </div>
            <div>
                <label>
                    <p class="product-price"><?= number_format($sp__Get_By_Id->dongia) ?> VND</p>
                </label>
            </div>
            ...
            <label for=""><b>Chọn kích thước:</b></label>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const slides = document.querySelectorAll('.carousel-wrapper input[name="slider"]');
        let currentSlide = 0;
        ...
        setInterval(showNextSlide, 3000); // Chuyển đổi ảnh mỗi 3 giây
    });
</script>
<script>
